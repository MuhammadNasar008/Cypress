...
Cypress.Commands.add('scrollUntilNoNewLists', () => {
    let previousRowCount = -1;
  
    cy.get("[data-view-type='list']>div>div").its('length').then(length => {
      previousRowCount = length;
    });
    cy.get("[data-view-type='list']>div>div").last().scrollIntoView();
    cy.wait(1000); 
    cy.get("[data-view-type='list']>div>div").its('length').then(length => {
