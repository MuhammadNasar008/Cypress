...
Cypress.Commands.add('checkCampaignOverview', (campaignId, campaignStatus, { campaignName, campaignType, campaignStartDate, campaignEndDate, campaignOwner }) => {
    ...
    cy.get('#tblCampaigns > thead > tr > th').should(($campaignsHeaders) => {
        expect($campaignsHeaders, '7 items').to.have.length(5)
        ...
        expect($campaignsHeaders.eq(2)).to.have.text('Owner')
        expect($campaignsHeaders.eq(3)).to.have.text('Status')
    })

    // campaign
    cy.get(`${specificLocationSelector(campaignId)} td`).should(($campaignsBody) => {
        expect($campaignsBody, '7 items').to.have.length(5)
        ...
        expect($campaignsBody.eq(2)).to.contain(campaignOwner)
        expect($campaignsBody.eq(3)).to.have.text(campaignStatus)
    })
})
